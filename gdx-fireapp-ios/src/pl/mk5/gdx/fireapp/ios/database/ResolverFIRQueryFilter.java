        ...
        switch (filterType) {
            ...
            case START_AT:
                if (filterArguments[0] instanceof Double) {
                    return target.queryStartingAtValue(NSNumber.valueOf((Double) filterArguments[0]));
                } else if (filterArguments[0] instanceof Boolean) {
                    return target.queryStartingAtValue(NSNumber.valueOf((Boolean) filterArguments[0]));
                } else if (filterArguments[0] instanceof String) {
                    return target.queryStartingAtValue(new NSString((String) filterArguments[0]));
                } else {
                    throw new IllegalArgumentException(WRONG_ARGUMENT_TYPE2);
                }
            case END_AT:
                if (filterArguments[0] instanceof Double) {
                    return target.queryEndingAtValue(NSNumber.valueOf((Double) filterArguments[0]));
                } else if (filterArguments[0] instanceof Boolean) {
                    return target.queryEndingAtValue(NSNumber.valueOf((Boolean) filterArguments[0]));
                } else if (filterArguments[0] instanceof String) {
                    return target.queryEndingAtValue(new NSString((String) filterArguments[0]));
                } else {
                    throw new IllegalArgumentException(WRONG_ARGUMENT_TYPE2);
                }
            case EQUAL_TO:
                if (filterArguments[0] instanceof Double) {
                    return target.queryEqualToValue(NSNumber.valueOf((Double) filterArguments[0]));
                } else if (filterArguments[0] instanceof Boolean) {
                    return target.queryEqualToValue(NSNumber.valueOf((Boolean) filterArguments[0]));
                } else if (filterArguments[0] instanceof String) {
                    return target.queryEqualToValue(new NSString((String) filterArguments[0]));
                } else {
                    throw new IllegalArgumentException(WRONG_ARGUMENT_TYPE2);
                }
            default:
